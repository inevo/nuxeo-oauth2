...
public class Root extends ModuleRoot {
    ...
    public Object doGet(
            @PathParam("serviceProviderName") String serviceProviderName,
            @QueryParam(WEOAuthConstants.CODE_URL_PARAMETER) String code,
            @QueryParam(WEOAuthConstants.ERROR_URL_PARAMETER) String error,
            @DefaultValue("false") @QueryParam(WEOAuthConstants.INSTALLED_APP_PARAMETER) boolean isInstalledApp) throws Exception {
    	
    	// Checking if there was an error such as the user denied access
        if (error != null && error.length() > 0) {
        	return Response.status(HttpServletResponse.SC_NOT_ACCEPTABLE)
    			.entity("There was an error: \""+error+"\".")
    			.build();
        }
